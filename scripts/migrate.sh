...
mysql -h db -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" <<-EOSQL
...
for file in $FILES; do
  ...
  applied=$(mysql -h db -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" -sse \
    "SELECT COUNT(*) FROM migrations_log WHERE filename = '$filename'")

  if [ "$applied" -eq 0 ]; then
    echo "${GREEN}🚀 Applying $filename...${NC}"
    if mysql -h db -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" < "$file"; then
